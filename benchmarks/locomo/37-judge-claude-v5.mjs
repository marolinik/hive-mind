#!/usr/bin/env node
// v5 judge — Claude Opus 4.7 self-judge over the v5-claude retrieval cell.
// Reads cell-retrieval-v5-claude.jsonl. Writes claude-judgments-v5-retrieval.jsonl.
// Same Mem0 verbatim "be generous" prompt as 33-judge-claude.mjs.
...
const OUT_FILE = resolve(OUT_DIR, 'claude-judgments-v5-retrieval.jsonl');
...
async function judgeClaude(question, gold, generated) {
  ...
  let verdict = null;
  const m = content.match(/\{[^{}]*"label"\s*:\s*"(CORRECT|WRONG)"[^{}]*\}/i);
  if (m) verdict = m[1].toUpperCase() === 'CORRECT' ? 1 : 0;
  else if (/\bCORRECT\b/.test(content) && !/\bWRONG\b/.test(content)) verdict = 1;
  else if (/\bWRONG\b/.test(content) && !/\bCORRECT\b/.test(content)) verdict = 0;
...
async function main() {
  ...
  if (!existsSync(ANS_FILE)) throw new Error(`No v5 answers — run 36-cell-retrieval-v5-claude.mjs first`);
  const rows = readFileSync(ANS_FILE, 'utf8').trim().split('\n')
    .map(l => JSON.parse(l)).filter(r => !r.error);
  if (rows.length === 0) throw new Error('No valid v5 answers');
  ...
  console.log(`[v5-judge] judge=${JUDGE_MODEL} answers=${rows.length} processing=${total - resumeFrom} resume_from=${resumeFrom}`);
  const tStart = Date.now();
  let n = 0, correct = 0, pf = 0;
  for (let i = resumeFrom; i < total; i++) {
    const r = rows[i];
    process.stdout.write(`  [${i + 1}/${total}] cat=${r.category_label} | `);
    let v;
    try { v = await judgeClaude(r.question, r.ground_truth, r.answer_content); }
    ...
    appendFileSync(OUT_FILE, JSON.stringify({
      idx: i, instance_id: r.instance_id, cell: 'retrieval-v5-claude',
