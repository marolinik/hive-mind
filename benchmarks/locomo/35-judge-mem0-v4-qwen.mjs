...
const ANS_FILE = resolve(DATA_DIR, 'answers', 'cell-retrieval-v4-qwen.jsonl');
const OUT_DIR = resolve(DATA_DIR, 'judgments');
const OUT_FILE = resolve(OUT_DIR, 'mem0-judgments-v4-qwen-retrieval.jsonl');
...
async function judge(q, g, ge) {
  const prompt = ACCURACY_PROMPT.replace('{question}', q).replace('{gold_answer}', g).replace('{generated_answer}', ge);
  const t0 = Date.now();
  ...
  let verdict = null;
  try { const p = JSON.parse(content); if (p.label === 'CORRECT') verdict = 1; else if (p.label === 'WRONG') verdict = 0; } catch {}
  return { verdict, raw: content, elapsed_ms };
}
...
async function main() {
  ...
  console.log(`[mem0-judge-v4-qwen] ${rows.length} answers | processing ${rows.length - resumeFrom} | resume=${resumeFrom}`);
  ...
  for (let i = resumeFrom; i < rows.length; i++) {
    ...
    appendFileSync(OUT_FILE, JSON.stringify({
      idx: i, instance_id: r.instance_id, cell: 'retrieval-v4-qwen',
