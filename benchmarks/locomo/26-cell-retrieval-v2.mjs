#!/usr/bin/env node
// Track A v2 retrieval — Fix #1 + #3 + #4 combined.
//
// Differences vs 22-cell-retrieval-gpt4o.mjs:
//   • K=10 semantic recall (Fix #4)
//   • Plus 5 importance-weighted frames via direct SQL ORDER BY importance (Fix #3)
//   • Combined retrieved context surfaces both raw turns AND categorized frames
//     (session-summaries, observations, events) added in 25-ingest-categorized
//   • Subject + judge + sample identical to baseline run for clean comparison
//
// Output: data/answers/cell-retrieval-gpt4o-v2.jsonl
...
const OUT_FILE = resolve(OUT_DIR, 'cell-retrieval-gpt4o-v2.jsonl');
...
const SYSTEM_PROMPT =
  ...
  'snippets retrieved from a memory system — these may include both raw dialogue turns ' +
  'AND distilled session summaries / observations. The snippets may not contain the full ' +
  'answer. Answer the question as concisely and precisely as possible based on what is ' +
  'given. If the answer is a date, format it as in the snippets. If the snippets do not ' +
  'contain enough information, say "Not stated in the retrieved context." Provide ONLY ' +
...
async function main() {
  ...
  console.log(`[retrieval-v2] subject=${SUBJECT_MODEL} K_sem=${SEMANTIC_K} K_imp=${IMPORTANCE_K} sample=${sample.length} total=${total} resume=${resumeFrom}`);
    ...
    for (let i = resumeFrom; i < total; i++) {
      ...
      try {
        const recall = await runRecallContext({
          query: it.question, scope: 'current', workspace: ws, limit: SEMANTIC_K,
          profile: 'balanced', env,
        });
        const importantFrames = fetchImportantFrames(MindDB, mindPath, IMPORTANCE_K);

        // Dedup by frame id, prefer importance-tagged frames first then semantic
        const seen = new Set();
        const combined = [];
