...
const ANS_FILE = resolve(DATA_DIR, 'answers', 'cell-retrieval-gpt4o-v4.jsonl');
const OUT_DIR = resolve(DATA_DIR, 'judgments');
const OUT_FILE = resolve(OUT_DIR, 'mem0-judgments-gpt4o-v4-retrieval.jsonl');
...
async function main() {
  ...
  console.log(`[mem0-judge-v4] ${rows.length} answers | processing ${rows.length - resumeFrom} | resume=${resumeFrom}`);
  ...
  for (let i = resumeFrom; i < rows.length; i++) {
    ...
    appendFileSync(OUT_FILE, JSON.stringify({
      idx: i, instance_id: r.instance_id, cell: 'retrieval-v4',
