#!/usr/bin/env node
// Phase 33 — Claude Opus 4.7 self-judge (Mem0 verbatim prompt)
// Judges the cell-{oracle,retrieval}-claude.jsonl answers.
// Same Mem0 "be generous" prompt + JSON output, just different judge model.
...
const OUT_FILE = resolve(OUT_DIR, 'claude-self-judgments.jsonl');
...
async function judgeClaude(question, gold, generated) {
  ...
  // Anthropic doesn't have native JSON mode; extract {"label":"..."} from response
  let verdict = null;
...
function loadAnswers() {
  const rows = [];
  for (const c of ['oracle', 'retrieval']) {
    const p = resolve(ANS_DIR, `cell-${c}-claude.jsonl`);
...
async function main() {
  ...
  if (rows.length === 0) throw new Error('No claude answers — run 31+32 first');
  ...
  console.log(`[claude-self-judge] judge=${JUDGE_MODEL} ${rows.length} answers; processing ${total - resumeFrom} (resume_from=${resumeFrom})`);
