#!/usr/bin/env node
// Track A — judge v2 retrieval answers with Mem0 verbatim protocol.
// Same as 23-judge-mem0-gpt4o.mjs but reads cell-retrieval-gpt4o-v2.jsonl
// and writes mem0-judgments-gpt4o-v2-retrieval.jsonl.
...
const ANS_FILE = resolve(DATA_DIR, 'answers', 'cell-retrieval-gpt4o-v2.jsonl');
const OUT_DIR = resolve(DATA_DIR, 'judgments');
const OUT_FILE = resolve(OUT_DIR, 'mem0-judgments-gpt4o-v2-retrieval.jsonl');
...
async function main() {
  ...
  if (rows.length === 0) throw new Error('No v2 answers — run 26 first');
  const resumeFrom = countLines(OUT_FILE);

  console.log(`[mem0-judge-v2] ${rows.length} answers | processing ${rows.length - resumeFrom} | resume=${resumeFrom}`);
  ...
  for (let i = resumeFrom; i < rows.length; i++) {
    ...
    appendFileSync(OUT_FILE, JSON.stringify({
      idx: i, instance_id: r.instance_id, cell: 'retrieval-v2',
